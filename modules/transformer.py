...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(
        ...
        self.fc1 = Linear(
            self.embed_dim, fc_hid_coeff * self.embed_dim
        )  # The "Add & Norm" part in the paper
        self.fc2 = Linear(fc_hid_coeff * self.embed_dim, self.embed_dim)
    ...
    def forward(self, x, x_k=None, x_v=None):
        ...
        if x_k is None and x_v is None:
            x, self.att_weight = self.self_attn(query=x, key=x, value=x, attn_mask=mask)
        else:
            x_k = self.maybe_layer_norm(0, x_k, before=True)
            x_v = self.maybe_layer_norm(0, x_v, before=True)
            x, self.att_weight = self.self_attn(
                query=x, key=x_k, value=x_v, attn_mask=mask
            )
